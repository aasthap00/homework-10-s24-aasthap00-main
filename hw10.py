...
def conf_matrix(y_pred, y_true, num_class):
    ...
    y_predicteed = map(int, y_pred)
    y_true_int = map(int, y_true)

    for i,j in zip(y_predicteed, y_true_int):
        confusion[i,j] += 1
...
def get_model_results(model_name, params, train_data, train_labels, test_data, test_labels, num_class):
    ...
    conf_mat = conf_matrix(test_data, test_labels, num_class)

    # 6. Compute the AUROC score. You may use metrics.roc_auc_score(...)
    if num_class > 2:
        probability = model.predict_proba(test_data)
        auc_score = roc_auc_score(test_labels, probability, multi_class='ovr')
    else:
        probability = model.predict_proba(test_data)[:, 1]
        auc_score = roc_auc_score(test_labels, probability)

    return acc, conf_mat, auc_score
